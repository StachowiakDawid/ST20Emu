...
namespace base_addr {
inline constexpr uint32_t video = 0x00000000;          // MPEG video decoder
inline constexpr uint32_t clock_gen = 0x00000100;      // Clock Generator
inline constexpr uint32_t audio = 0x00000200;          // Audio decoder
inline constexpr uint32_t sub_picture = 0x00000400;    // Sub-picture decoder
inline constexpr uint32_t denc = 0x00000600;           // Digital Encoder
inline constexpr uint32_t mpeg_ctrl = 0x00000E00;      // MPEG Control (0xE00 0xE01)
inline constexpr uint32_t emi = 0x00002000;            // Programmable CPU Memory Interface
inline constexpr uint32_t cache = 0x00004000;          // Cache configuration
inline constexpr uint32_t int_controller = 0x20000000; // Interrupt controller
inline constexpr uint32_t dkey = 0x20002500;           // Descrambling KEYS
inline constexpr uint32_t lpc = 0x20000400;            // Low power and Watchdog
// Asynchronous serial controllers
inline constexpr uint32_t asc0 = 0x20003000; // ASC0 / SmartCard 0
inline constexpr uint32_t asc1 = 0x20004000; // ASC1
inline constexpr uint32_t asc2 = 0x20005000; // ASC2
inline constexpr uint32_t asc3 = 0x20006000; // ASC3 / SmartCard 1
// Synchronous serial controllers
inline constexpr uint32_t ssc0 = 0x20009000; // SSC0
inline constexpr uint32_t ssc1 = 0x2000A000; // SSC1
// Other controllers
inline constexpr uint32_t pio5 = 0x2000A100;      // PIO port 5 controller
inline constexpr uint32_t irb = 0x2000A200;       // Infra red blaster
inline constexpr uint32_t ttxt = 0x2000A300;      // Teletext
inline constexpr uint32_t pwm = 0x2000B000;       // PWM and counter module
inline constexpr uint32_t pio0 = 0x2000C000;      // PIO port 0 controller
inline constexpr uint32_t pio1 = 0x2000D000;      // PIO port 1 controller
inline constexpr uint32_t pio2 = 0x2000E000;      // PIO port 2 controller
inline constexpr uint32_t pio3 = 0x2000F000;      // PIO port 3 controller
inline constexpr uint32_t pio4 = 0x20010000;      // PIO port 4 controller
inline constexpr uint32_t int_level = 0x20011000; // Interrupt level controller
inline constexpr uint32_t mpeg_dma0 = 0x20024000; // MPEGDMA controller
inline constexpr uint32_t mpeg_dma1 = 0x20025000; // MPEGDMA controller
inline constexpr uint32_t bm = 0x20026000;        // Block move DMA controller
inline constexpr uint32_t modem = 0x20027000;     // Modem control
inline constexpr uint32_t mpeg_dma2 = 0x20030000; // LINK (SDAV MPEGDMA2) controller
inline constexpr uint32_t link = 0x20038000;      // Link and front-end interface
} // namespace base_addr
...
// To count the number of registers just copy the inside of this array
// then use CTRL + F to find: '   {"'
// this trick ignores commented out registers e.g.: '   //{"'

// Notice: some of the registers have the same address,
// so they are commented out in this lookup table
inline constexpr std::array<RegisterDescription, 1046> db_regs_entries = {
    {// VIDEO DECODER (VID)
     ...
     {"AUD_LDLY__AUD_TM_SPEED", 0x00000257, 8, "R/W", "Left channel / Audio trick-mode speed"},
     ...
     {"AUD_PCM_BTONE__AUD_DECODE_LFE__AUD_MP_SKIP_LFE", 0x00000268, 8, "R/W",
      "PCM beep tone frequency / Decode LFE for AC3 setup / Channel skip for MPEG setup"},
     ...
     {"AUD_COMP_MOD__AUD_MP_PROG_NO", 0x00000269, 8, "R/W",
      "Compression mode for AC3 setup / Program number for MPEG setup"},
     ...
     {"AUD_AC3_HDR__AUD_MP_DRC", 0x0000026A, 8, "R/W",
      "High dynamic range for AC3 setup / High dynamic range for MPEG setup (1 bit)"},
     ...
     {"AUD_AC3_CRC_OFF__AUD_MP_CRC_OFF", 0x0000026C, 8, "R/W",
      "Repeat count for AC3 setup / CRC check-off for MPEG setup"},
     //{"AUD_AC3_CRC_OFF", 0x0000026C, 8, "R/W", "Repeat count for AC3 setup"},
     //{"AUD_MP_CRC_OFF", 0x0000026C, 8, "R/W", "CRC check-off for MPEG setup"},
     {"AUD_AC3_KARAMODE__AUD_MP_MC_OFF", 0x0000026D, 8, "R/W",
      "Karaoke downmix for AC3 setup (8b) / Multichannel for MPEG setup (2b)"},
     ...
     {"AUD_AC3_DUALMODE__AUD_MP_DUAL", 0x0000026E, 8, "R/W",
      "Dual downmix for AC3 setup / Dual downmix for MPEG setup"},
     //{"AUD_AC3_DUALMODE", 0x0000026E, 8, "R/W", "Dual downmix for AC3 setup"},
     //{"AUD_MP_DUAL", 0x0000026E, 8, "R/W", "Dual downmix for MPEG setup"},
     {"AUD_AC3_DOWNMIX__AUD_MP_DOWNMIX", 0x0000026F, 8, "R/W",
      "Downmix for AC3 setup (8b) / Downmix for MPEG setup (6b)"},
     ...
     {"AUD__MP__LPCM_STATUS0", 0x00000276, 8, "RO",
      "AC-3 (8b) / MPEG (8b) / LPMC (7b) status register 0"},
     //{"AUD_STATUS0", 0x00000276, 8, "RO", "AC-3 status register 0"},
     //{"AUD_MP_STATUS0", 0x00000276, 8, "RO", "MPEG status register0"},
     //{"AUD_LPCM_STATUS0", 0x00000276, 7, "RW", "LPMC status register0"},
     {"AUD__MP__LPCM_STATUS1", 0x00000277, 4, "RO",
      "AC-3 (4b) / MPEG (4b) / LPMC (7b) status register 1"},
     ...
     {"AUD__MP__LPCM_STATUS2", 0x00000278, 8, "RO",
      "AC-3 (8b) / MPEG (4b) / LPMC (8b) status register 2"},
     ...
     {"OGT_CTL", base_addr::sub_picture + 0x000040, 7, "R/W", "Control register"},
     {"OGT_LUT", base_addr::sub_picture + 0x000041, 8, "R/W", "Main look-up table"},
     {"OGT_LUT_H1", base_addr::sub_picture + 0x000042, 8, "R/W", "Highlight 1 look-up table"},
     {"OGT_LUT_H2", base_addr::sub_picture + 0x000043, 8, "R/W", "Highlight 2 look-up table"},
     {"OGT_XDI", base_addr::sub_picture + 0x000044, 2, "R/W",
      "Active area horizontal position offset"},
     {"OGT_XDI", base_addr::sub_picture + 0x000045, 8, "R/W",
      "Active area horizontal position offset"},
     {"OGT_YDI", base_addr::sub_picture + 0x000046, 2, "R/W",
      "Active area vertical position offset"},
     {"OGT_YDI", base_addr::sub_picture + 0x000047, 8, "R/W",
      "Active area vertical position offset"},
     {"OGT_HL2XO", base_addr::sub_picture + 0x000048, 2, "R/W",
      "Active area vertical position offset"},
     {"OGT_HL2XO", base_addr::sub_picture + 0x000049, 8, "R/W",
      "Active area vertical position offset"},
     {"OGT_HL2YO", base_addr::sub_picture + 0x00004A, 2, "R/W", "Highlight 2 area, start Y"},
     {"OGT_HL2YO", base_addr::sub_picture + 0x00004B, 8, "R/W", "Highlight 2 area, start Y"},
     {"OGT_HL2XI", base_addr::sub_picture + 0x00004C, 2, "R/W", "Highlight 2 area, end X"},
     {"OGT_HL2XI", base_addr::sub_picture + 0x00004D, 8, "R/W", "Highlight 2 area, end X"},
     {"OGT_HL2YI", base_addr::sub_picture + 0x00004E, 2, "R/W", "Highlight 2 area, end Y"},
     {"OGT_HL2YI", base_addr::sub_picture + 0x00004F, 8, "R/W", "Highlight 2 area, end Y"},
     {"OGT_STAT", base_addr::sub_picture + 0x000050, 2, "R", "Status register"},
     ...
     {"EMI_CONFIGDATA0BANK0", 0x00002000, 16, "R/W",
      "EMI bank 0 configuration data register 0 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA1BANK0", 0x00002004, 16, "R/W",
      "EMI bank 0 configuration data register 1 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA2BANK0", 0x00002008, 16, "R/W",
      "EMI bank 0 configuration data register 2 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA3BANK0", 0x0000200c, 16, "R/W",
      "EMI bank 0 configuration data register 3 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA0BANK1", 0x00002010, 16, "R/W",
      "EMI bank 1 configuration data register 0 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA1BANK1", 0x00002014, 16, "R/W",
      "EMI bank 1 configuration data register 1 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA2BANK1", 0x00002018, 16, "R/W",
      "EMI bank 1 configuration data register 2 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA3BANK1", 0x0000201c, 16, "R/W",
      "EMI bank 1 configuration data register 3 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA0BANK2", 0x00002020, 16, "R/W",
      "EMI bank 2 configuration data register 0 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA1BANK2", 0x00002024, 16, "R/W",
      "EMI bank 2 configuration data register 1 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA2BANK2", 0x00002028, 16, "R/W",
      "EMI bank 2 configuration data register 2 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA3BANK2", 0x0000202c, 16, "R/W",
      "EMI bank 2 configuration data register 3 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA0BANK3", 0x00002030, 16, "R/W",
      "EMI bank 3 configuration data register 0 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA1BANK3", 0x00002034, 16, "R/W",
      "EMI bank 3 configuration data register 1 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA2BANK3", 0x00002038, 16, "R/W",
      "EMI bank 3 configuration data register 2 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGDATA3BANK3", 0x0000203c, 16, "R/W",
      "EMI bank 3 configuration data register 3 SDRAM, DRAM and peripheral memory formats"},
     {"EMI_CONFIGLOCKBANK0", 0x00002040, 1, "W",
      "Write protection bit. When set, makes EMI_ConfigData 0-3 for Bank0 read only"},
     {"EMI_CONFIGLOCKBANK0", 0x00002044, 1, "W",
      "Write protection bit. When set, makes EMI_ConfigData 0-3 for Bank1 read only"},
     {"EMI_CONFIGLOCKBANK0", 0x00002048, 1, "W",
      "Write protection bit. When set, makes EMI_ConfigData 0-3 for Bank2 read only"},
     {"EMI_CONFIGLOCKBANK0", 0x0000204C, 1, "W",
      "Write protection bit. When set, makes EMI_ConfigData 0-3 for Bank3 read only"},
     {"EMI_CONFIGSTATUS", 0x00002050, 8, "R", "Status information"},
     {"EMI_SDRAMMODEREG0", 0x00002058, 7, "W",
      "SDRAM Mode Register for either the case of one SDRAM bank or SDRAM in bank0, when two SDRAM "
      "banks."},
     ...
     {"IRB_TX_PRE_SCALER_IR", base_addr::irb + 0x00, 8, "W/R", "Clock pre-scaler selection"},
     {"IRB_TX_SUB_CARRIER_IR", base_addr::irb + 0x04, 16, "W/R", "sub-carrier freq. programming"},
     {"IRB_TX_SYM_TIME_IR1", base_addr::irb + 0x08, 16, "W", "symbol time programming"},
     {"IRB_TX_ON_TIME_IR1", base_addr::irb + 0x0C, 16, "W", "symbol on time programming"},
     {"IRB_TX_INT_EN_IR", base_addr::irb + 0x10, 3, "W/R", "Transmit Interrupt enable register"},
     {"IRB_TX_INT_STATUS_IR", base_addr::irb + 0x14, 4, "R", "Transmit Interrupt status register"},
     {"IRB_TX_EN_IR", base_addr::irb + 0x18, 1, "W/R", "RC transmit enable register"},
     {"IRB_TX_CLR_UNDERRUN_IR", base_addr::irb + 0x1C, 1, "W", "Clears the underrun status"},
     {"IRB_RX_ON_TIME_IR1", base_addr::irb + 0x40, 16, "R", "Received pulse time capture RC"},
     {"IRB_RX_SYM_TIME_IR1", base_addr::irb + 0x44, 16, "R", "Received symbol time capture"},
     {"IRB_RX_INT_EN_IR", base_addr::irb + 0x48, 4, "W/R", "Receive Interrupt enable register"},
     {"IRB_RX_INT_STATUS_IR", base_addr::irb + 0x4C, 5, "R", "Receive Interrupt status register"},
     {"IRB_RX_EN_IR", base_addr::irb + 0x50, 1, "W/R", "RC receive enable register"},
     {"IRB_RX_MAX_SYM_TIME_IR", base_addr::irb + 0x54, 16, "W/R",
      "Maximum RC symbol time register"},
     {"IRB_RX_CLR_OVERRUN_IR", base_addr::irb + 0x58, 1, "W", "Clears the overrun status"},
     {"IRB_RX_NOISE_SUPPRESS_WIDTH_IR", base_addr::irb + 0x5C, 8, "W/R", "Reserved"},
     {"IRB_RC_IRDA_CONTROL", base_addr::irb + 0x60, 1, "W/R", "Reserved"},
     {"IRB_RX_SAMPLING_RATE_COMMON", base_addr::irb + 0x64, 4, "W/R",
      "Sampling frequency division for UHF and IR frequencies."},
     {"IRB_RX_ON_TIME_UHF1", base_addr::irb + 0x80, 16, "R", "Received pulse time capture"},
     {"IRB_RX_SYM_TIME_UHF1", base_addr::irb + 0x84, 16, "R", "Received symbol time capture"},
     {"IRB_RX_INT_EN_UHF", base_addr::irb + 0x88, 4, "W/R", "Receive Interrupt enable register"},
     {"IRB_RX_INT_STATUS_UHF", base_addr::irb + 0x8C, 5, "R", "Receive Interrupt status register"},
     {"IRB_RX_EN_UHF", base_addr::irb + 0x90, 1, "W/R", "RC receive enable register"},
     {"IRB_RX_MAX_SYM_TIME_UHF", base_addr::irb + 0x94, 16, "W/R",
      "Maximum RC symbol time register"},
     {"IRB_RX_CLR_OVERRUN_UHF", base_addr::irb + 0x98, 1, "W", "Clears the overrun status"},
     {"IRB_RX_NOISE_SUPPRESS_WIDTH_UHF", base_addr::irb + 0x9C, 8, "W/R",
      "Noise suppression width"},
     ...
     {"INC_SELNOTINV", 0x200111C8, 3, "R/W",
      "External interrupt pin is set to active high or active low"},
     {"INC_EN_INT", 0x200111CC, 3, "R/W",
      "Enable external interrupt to wake-up the low-power controller"},
     ...
     // MPEGDMA0 (MPEGDMA)
     {"MPEGDMA0_BURSTSIZE", 0x20024000, 5, "W",
      "The number of bytes to be transferred in one burst"},
     {"MPEGDMA0_HOLDOFF", 0x20024004, 5, "W",
     ...
     // MPEGDMA1 (MPEGDMA)
     {"MPEGDMA1_BURSTSIZE", 0x20025000, 5, "W",
      "The number of bytes to be transferred in one burst"},
     {"MPEGDMA1_HOLDOFF", 0x20025004, 5, "W",
      "Holdoff for MPEG decoder: range 0 to 31, where 0=0 delay cycles"},
     ...
     // TODO: check these LNK registers if they are correct/can be defined better
     // LNK REGS
     //{"LNK_STREAM1_CONF",0x20038580,32,"R/W","Link configuration 1"},
     {"LNK_STREAM_FRAM", 0x200384FC, 32, "R/W",
      "End of Filter Data (FRAM) register - 320words from LNKBaseAddress"},
     ...
     {"LNK_STREAM1_CONF10", 0x200385A4, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF11", 0x200385A8, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF12", 0x200385AC, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF13", 0x200385B0, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF14", 0x200385B4, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF15", 0x200385B8, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF16", 0x200385BC, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF17", 0x200385C0, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF18", 0x200385C4, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF19", 0x200385C8, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF20", 0x200385CC, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF21", 0x200385D0, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF22", 0x200385D4, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF23", 0x200385D8, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF24", 0x200385DC, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF25", 0x200385E0, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF26", 0x200385E4, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF27", 0x200385E8, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF28", 0x200385EC, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF29", 0x200385F0, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF30", 0x200385F4, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF31", 0x200385F8, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM1_CONF32", 0x200385FC, 32, "R/W",
      "Stream Configuration #1 (see pag.176 Reg.Man.)"},
     ...
     {"LNK_STREAM2_CONF10", 0x20038624, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF11", 0x20038628, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF12", 0x2003862C, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF13", 0x20038630, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF14", 0x20038634, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF15", 0x20038638, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF16", 0x2003863C, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF17", 0x20038640, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF18", 0x20038644, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF19", 0x20038648, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF20", 0x2003864C, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF21", 0x20038650, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF22", 0x20038654, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF23", 0x20038658, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF24", 0x2003865C, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF25", 0x20038660, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF26", 0x20038664, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF27", 0x20038668, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF28", 0x2003866C, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF29", 0x20038670, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF30", 0x20038674, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF31", 0x20038678, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM2_CONF32", 0x2003867C, 32, "R/W",
      "Stream Configuration #2 (see pag.176 Reg.Man.)"},
     ...
     {"LNK_STREAM3_CONF10", 0x200386A4, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF11", 0x200386A8, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF12", 0x200386AC, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF13", 0x200386B0, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF14", 0x200386B4, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF15", 0x200386B8, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF16", 0x200386BC, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF17", 0x200386C0, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF18", 0x200386C4, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF19", 0x200386C8, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF20", 0x200386CC, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF21", 0x200386D0, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF22", 0x200386D4, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF23", 0x200386D8, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF24", 0x200386DC, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF25", 0x200386E0, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF26", 0x200386E4, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF27", 0x200386E8, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF28", 0x200386EC, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF29", 0x200386F0, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF30", 0x200386F4, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF31", 0x200386F8, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     {"LNK_STREAM3_CONF32", 0x200386FC, 32, "R/W",
      "Stream Configuration #3 (see pag.176 Reg.Man.)"},
     ...
     {"FEI_DEC_IDX__FEI_DEC_STA", 0x20038784, 6, "R/W",
      "Decryption Cell Register Index (WriteOnly:6b) / DCR Status (ReadOnly:2)"},
     //{"FEI_DEC_IDX", 0x20038784, 6, "R/W", "Decryption Cell Register Index"},
     ...
     {"FEI_SLG", 0x20038798, 16, "R/W",
      "Only used when data are input using the parallel interface"},
     ...
     {"FEI_SE_CAP_H", 0x20038820, 8, "R/W",
      "Address of first sector /subcode block to be captured"},
     {"FEI_SE_CAP_L", 0x20038824, 8, "R/W",
      "Address of first sector /subcode block to be captured"},
     {"FEI_SE_LST_H", 0x20038828, 8, "R/W",
      "Address of (last + 1) sector /subcode block to be captured"},
     {"FEI_SE_LST_L", 0x2003882C, 16, "R/W",
      "Address of (last + 1) sector/subcode block to be captured"},
     {"FEI_SE_CUR_H", 0x20038830, 8, "RO",
      "Address of sector/subcode block actually entering the SP"},
     {"FEI_SE_CUR_L", 0x20038834, 16, "RO", "Address of sector /subcode block entering the SP"},
     {"FEI_SE_HE_H", 0x20038860, 16, "R/W",
      "CPR_MAI Bits of Current Sector actually entering the SP"},
     {"FEI_SE_HE_M", 0x20038864, 16, "R/W",
      "CPR_MAI Bits of Current Sector actually entering the SP"},
     {"FEI_SE_HE_L", 0x20038868, 16, "R/W",
      "CPR_MAI Bits of Current Sector actually entering the SP"},
     ...
     {"LNK_AF0", 0x20038F98, 32, "R", "Adaptation field bytes 0 to 3"},
     {"LNK_AF1", 0x20038F9C, 32, "R", "Adaptation field bytes 4 to 7"},
